...
import com.Mortgage.MortgageLoanAPI.dao.IUserDao;
import com.Mortgage.MortgageLoanAPI.models.User;
import com.Mortgage.MortgageLoanAPI.utils.Constants;
import com.Mortgage.MortgageLoanAPI.utils.PasswordUtil;
...
public class UserBo {
	...
	public Map<String, Object> saveUpdate(User app) {
		...
		try {
			...
			if (str != null) { 
				...
				send.email(Constants.EMAIL_SUBJECT_PROFILE, app.getEmail(), "Your password : "+pwd);
			}
	...
	public Map<String, String> forgotPassword(String email) throws Exception {
		...
		if (stud.size() > 0) {
			...
			SendEmail send = new SendEmail();
			String message = send.email(Constants.EMAIL_SUBJECT_PROFILE, email, "Your password : "+pass);
