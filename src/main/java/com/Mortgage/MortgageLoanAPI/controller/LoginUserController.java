...
import com.Mortgage.MortgageLoanAPI.bo.UserBo;
import com.Mortgage.MortgageLoanAPI.configuration.LoginRequired;
import com.Mortgage.MortgageLoanAPI.models.Login;
import com.Mortgage.MortgageLoanAPI.models.Password;
import com.Mortgage.MortgageLoanAPI.models.User;
import com.Mortgage.MortgageLoanAPI.utils.Constants;
import com.Mortgage.MortgageLoanAPI.utils.JWTUtil;

@RestController
@RequestMapping("/user-ctr")
public class LoginUserController {
	...
	public ResponseEntity<Map<String, Object>> login(@RequestBody Login login) {
		...
		try {
			User s = userBo.login(login.getEmail(), login.getPassword());
			if(s.getId() != null) {
	...
	public ResponseEntity<Map<String, String>> forgotPassword(@PathVariable String email) {
		...
		try {
			rsMap = userBo.forgotPassword(email);
	...
	public ResponseEntity<Map<String, String>> changePassword(@RequestBody Password pwd, @PathVariable("id") Long id) {
		...
		try {
			rsMap = userBo.changePassword(id, pwd.getOldPassword(), pwd.getNewPassword());
	...
	public ResponseEntity<Map<String,Object>>  findByEmail(@PathVariable("email") String email){
		...
		Map<String, Object> rsMap = new HashMap<>();
		try {
			rsMap = userBo.findByEmail(email);
